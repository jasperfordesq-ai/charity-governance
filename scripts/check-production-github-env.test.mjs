...
test('production GitHub environment check fails for missing and placeholder variables', () => {
  ...
  assert.match(
    result.stderr,
    /GitHub production variable NEXT_PUBLIC_API_URL must not contain placeholder text/,
  );
  assert.match(
    result.stderr,
  ...
  assert.doesNotMatch(result.stderr, /REPLACE_ME_API_ORIGIN/);
});

test('production GitHub environment check validates the canonical API origin', () => {
  const result = runProductionGitHubEnvironmentCheckFromArgs([], {
    runGh: () =>
      okGh([
...
test('production GitHub environment check renders redacted JSON for automation', () => {
  ...
  assert.doesNotMatch(result.stdout, /REPLACE_ME_API_ORIGIN/);
});
