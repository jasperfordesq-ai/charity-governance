...
test('production deploy smoke rejects placeholder readiness keys before fetching', async () => {
  const runProductionDeploySmokeFromArgs = await loadSmokeRunner();
  const tempDir = mkdtempSync(join(tmpdir(), 'charitypilot-production-smoke-placeholder-readiness-'));
  const envPath = join(tempDir, 'production.env');
  let fetchCalled = false;

  writeFileSync(envPath, completeSmokeEnv({
    READINESS_API_KEY: 'REPLACE_ME_READINESS_KEY',
  }));
  ...
  try {
    ...
    assert.equal(result.status, 1);
    assert.equal(fetchCalled, false);
    assert.match(result.stderr, /READINESS_API_KEY is required for production deploy smoke/);
  } finally {
    rmSync(tempDir, { recursive: true, force: true });
  }
});
