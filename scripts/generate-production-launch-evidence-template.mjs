...
const evidenceHintsByCheck = new Map([
  ...
  ['releaseGate.deploy-rollback', [
    'separate command-output evidence: same-line P1-07A-compatible rollback rehearsal',
    'npm run deploy:rollback -- --production-env-file=.env.production --rollback-digest-file=release-image-digests.previous.env --schema-compatibility-attestation-file=<protected-attestation> --backup-output-dir=<approved-encrypted-path>',
    'separate command-output evidence: failed P1-09 recovery rehearsal',
    ...
    'separate command-output evidence: failed P1-07A recovery rehearsal',
    ...
    'separate command-output evidence: P1-09 restore-only cross-boundary rollback rehearsal',
    'npm run deploy:rollback -- --production-env-file=.env.production --rollback-digest-file=release-image-digests.previous.env --database-restore-attestation-file=<protected-attestation> --restored-backup-file=<exact-protected-backup> --backup-output-dir=<approved-encrypted-path>',
    'p109-restored',
  ...
  ['releaseGate.digest-manifest', ['release-image-digests.env', 'CHARITYPILOT_API_IMAGE', 'CHARITYPILOT_WEB_IMAGE', 'CHARITYPILOT_MIGRATION_IMAGE', 'CHARITYPILOT_DATABASE_COMPATIBILITY=p107a-password-recovery-v1']],
  ...
  ['secretsAndEnv.jwt-secret-entropy', ['JWT_SECRET', 'AUTH_RECOVERY_SECRET', '32 to 64 bytes', 'independent', 'high entropy']],
  ['secretsAndEnv.auth-recovery-secret-rotation-rehearsal', [
    'four distinct command-output entries from an isolated recent restore',
    'release.commitSha and the promoted digest-pinned API image in every transcript',
    ...
    'no raw secret, database URL, recipient, token, request, or account value',
  ]],
  ...
  ['jobs.auth-email-delivery-runtime', [
    'separate command-output evidence for deployed authentication email delivery runtime in production-scheduler',
    'release.commitSha and the promoted digest-pinned API image in both transcripts',
    'node dist/jobs/process-auth-email-delivery.js',
    'digest-pinned API image',
    'same production secret source',
    'separate command-output rehearsal against isolated non-production data',
    '[AuthEmailDelivery] completed',
    'recovery control ready',
    'exit 0',
  ]],
  ['jobs.auth-delivery-anomaly-alert', [
    'authentication-delivery anomaly alert rehearsal against isolated non-production data',
    'release.commitSha in the rehearsal and incident-system confirmation',
    'REJECTED',
    'UNCERTAIN',
    'KEY_UNAVAILABLE',
    'STALE_QUARANTINED',
  ...
  ['billingAndEmail.password-recovery-resend-delivery', [
    'EMAIL_FROM',
    'two distinct evidence entries from the deployed application',
    'release.commitSha in both deployed provider proof entries',
    'Resend accepted deployed recovery-link email',
    ...
    'complete fragment link to https://app.charitypilot.ie',
