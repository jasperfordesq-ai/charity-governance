...
test('production launch evidence status reports pending template progress without approving launch', async () => {
  ...
  try {
    ...
    assert.match(result.stdout, /Checklist checks complete: 0 \/ 89 \(0% complete\)/);
...
test('production launch evidence status counts completed checks and keeps final approval separate', async () => {
  ...
  try {
    ...
    assert.equal(result.status, 0);
    assert.match(result.stdout, /Checklist checks complete: 1 \/ 89 \(1\.1% complete\)/);
    assert.match(result.stdout, /Final approval roles approved: 0 \/ 5 \(0% complete\)/);
...
test('production launch evidence status renders non-secret JSON for automation', async () => {
  ...
  try {
    ...
    assert.equal(payload.totalChecks, 89);
    ...
    assert.equal(payload.incompleteCheckCount, 88);
    assert.deepEqual(
...
test('production launch evidence status refuses strict completion for generic evidence entries', async () => {
  ...
  try {
    ...
    assert.equal(result.status, 0);
    const payload = JSON.parse(result.stdout);
    assert.equal(payload.completedChecks, 89);
