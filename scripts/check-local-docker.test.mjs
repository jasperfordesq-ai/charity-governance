...
test('responsive smoke retries only bounded local web transport failures', () => {
  ...
  assert.match(navigationHelper, /Navigation to \$\{url\} returned HTTP \$\{status\}/);
  assert.match(navigationHelper, /nextDevOverlaySnippet/);
  assert.match(navigationHelper, /observePageErrors/);
  assert.match(navigationHelper, /page\.on\('pageerror'/);
  assert.match(navigationHelper, /browser JavaScript error before principle links rendered/);
  ...
  assert.match(navigationHelper, /COMPLIANCE_DETAIL_LINK_TIMEOUT_MS = 180_000/);
  assert.match(navigationHelper, /while \(Date\.now\(\) < deadline\)/);
  assert.match(navigationHelper, /Loading dashboard/);
  assert.match(navigationHelper, /redirected to login/);
...
test('platform audit ledger records local browser evidence without closing deployed gates', () => {
  ...
  assert.match(auditGenerator, /P1-07A is CI_VERIFIED[\s\S]*shared 55\/55[\s\S]*API 810\/810/);
  assert.match(auditLedger, /P1-07A is CI_VERIFIED[\s\S]*shared 55\/55[\s\S]*API 810\/810/);
  ...
  assert.match(auditLedger, /managed disposable E2E gate with 113\/113 runner contracts plus 105\/105 browser scenarios in 7\.6m followed by clean isolated teardown/);
  assert.match(auditLedger, /Implementation commit 1e639c89b49ce5ed27a8ea3b887ef140c7f142b5/);
  assert.match(auditLedger, /CI run 29184769464, which failed only at fresh scheduled-job image-smoke recovery binding/);
  assert.match(auditLedger, /companion E2E run 29184769502 passed/);
  assert.match(auditLedger, /exact final verification SHA b2138acfe0b7b7a9127a14667f10a771982a0e3b passed CI run 29185333589 in 8m24s/);
  assert.match(auditLedger, /managed E2E run 29185333588 in 6m41s with 105\/105 browser scenarios/);
  assert.match(auditLedger, /repository publication evidence, not deployed-browser, production-provider, legal, human-policy, or production-launch evidence/);
...
test('platform audit ledger records launch evidence gate hardening', () => {
  ...
  assert.match(auditLedger, /Upload workflow file: `\.github\/workflows\/upload-production-launch-evidence\.yml`/);
  assert.match(auditLedger, /Prepare\/upload: `npm run prepare:production:evidence-upload -- --json \| gh workflow run upload-production-launch-evidence\.yml --ref master --json`/);
  assert.match(auditLedger, /Upload evidence target: Use the successful upload-production-launch-evidence\.yml run id as evidence_artifact_run_id/);
  ...
  assert.match(auditLedger, /Validation artifact files:/);
