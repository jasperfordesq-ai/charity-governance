...
- Marketing blog search uses the shared empty-state primitive for no-result filters; marketing landing signal tiles use shared status panel styling; compliance standard autosave, organisation profile saving, governance register saving, document vault mutations, export board sign-off, and board/deadline/team mutations use the shared save-status primitive; billing checkout and portal handoffs use shared visible status; billing plan prices use flat bands instead of nested card panels; team permission-denied messages use shared permission hints instead of route-local hidden or bespoke status markup.
...
- Irish compliance source metadata was refreshed against official sources on 2026-07-06.
...
- `npm run release:ready -- --no-e2e`
  - Passed on 2026-07-08 at commit `dc26da0`.
