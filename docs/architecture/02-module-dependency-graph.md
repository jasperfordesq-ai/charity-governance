...
|---|---|---|---|---|---|
| **auth** | `/api/v1/auth` | `POST /register`, `POST /login`, `POST /refresh`, `POST /logout`, `GET /me`, `POST /forgot-password`, `POST /resend-verification`, `POST /reset-password`, `POST /verify-email` | `AuthService` (`apps/api/src/routes/auth/index.ts:33`) | `User` (`apps/api/src/services/auth.service.ts:69`); `AuthSession`, `User` via `session-tokens` (`apps/api/src/services/session-tokens.ts:52,97`) | `authIdentityGuard` on `/me` and `/resend-verification` only (`apps/api/src/routes/auth/index.ts:126,157`); per-route `rateLimit` config (e.g. `:37`). No org-level guards. |
...
| **compliance** | `/api/v1/compliance` | `GET /principles`, `GET /principles/:principleId`, `GET /records`, `GET /records/:standardId`, `PUT /records/:standardId`, `GET /summary`, `GET /signoff`, `PUT /signoff` | `ComplianceService` (`apps/api/src/routes/compliance/index.ts:18`) | `Organisation`, `Subscription`, `GovernanceStandard`, `GovernancePrinciple`, `ComplianceRecord`, `ComplianceSignoff` (`apps/api/src/services/compliance.service.ts:31-307`) | `authGuard` + `subscriptionGuard` (`:20-21`); `requireAdmin` on the two write routes (`:78,123`) |
...
| **export** | `/api/v1/export` | `GET /compliance-record`, `GET /compliance-report` (alias) | `ComplianceService` (`apps/api/src/routes/export/index.ts:10`); also reads several models directly via `app.prisma` | `Organisation`, `Subscription` directly (`apps/api/src/routes/export/index.ts:18,25`); `ConflictRecord`, `RiskRecord`, `ComplaintRecord`, `FundraisingRecord`, `AnnualReportReadiness`, `FinancialControlReview` directly when plan is COMPLETE (`apps/api/src/routes/export/index.ts:75-82`); plus all ComplianceService models | `authGuard` + `subscriptionGuard` (`:12-13`). Governance-register section gated in-handler by `subscription.plan === COMPLETE` (`:30`) |
