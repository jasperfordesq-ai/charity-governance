...
| **documents** | `/api/v1/documents` | `GET /`, `GET /:id`, `GET /:id/download` (authenticated byte proxy), `POST /` (multipart upload), `DELETE /:id`, `POST /:id/link-standard` (+ alias `POST /:id/standards`), `DELETE /:id/unlink-standard` (+ alias `DELETE /:id/standards/:standardId`) | `DocumentService` + `StorageService` | `Document`, `DocumentStandardLink`, `GovernanceStandard`, `Organisation`, `Subscription` | `authGuard` + `subscriptionGuard`; `requireAdmin` on POST/DELETE/link/unlink |
...
| **billing** | `/api/v1/billing` | `POST /webhooks` (raw-body Stripe), Checkout/Portal aliases, `GET /status` | `BillingService` + billing-authority interlock | `StripeWebhookEvent`, `Organisation`, `Subscription`, `BillingCheckoutAttempt`, `BillingAuthorityGrant`, `User`, `AuthSession` | Webhook is unauthenticated but signature-verified. Authenticated Checkout/Portal requires the current live owner and exact session under serializable locks; a durable grant is written before provider I/O. There is intentionally no `subscriptionGuard`, so terminal/recovery billing paths remain reachable. |
...
| **team** | `/api/v1/team` | Public invite acceptance; authed team/invite reads, invite/revoke, role/lifecycle actions, ownership transfer, session-family inventory/revocation, and security audit | `TeamService` + `TeamLifecycleService`; composes `EmailService` and `session-tokens` | `Organisation`, `Subscription`, `User`, `TeamInvite`, `AuthSession`, `DeadlineReminderLog`, `SecurityAuditEvent`, `BillingAuthorityGrant` | `/accept-invite` is identifier-rate-limited and rechecks active org/subscription/capacity generically. Authed scope uses `authGuard`; only new invitations add `subscriptionGuard`, so security/offboarding remains available during billing incidents. Every mutation reauthorises live tenant roles under locks and uses optimistic membership versions. |
