...
| `CHARITYPILOT_DATABASE_COMPATIBILITY` | release manifest + rollback guard | Declares the reviewed application/database compatibility line. `p006-deadline-calendar-v1` still requires a fresh exact-manifest-hash-bound operator attestation and live migration-history probe for image rollback; the marker is never sufficient by itself. | Deployment-specific |
