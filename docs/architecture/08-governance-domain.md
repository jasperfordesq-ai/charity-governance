...
`Deadline` records current and historical governance-calendar occurrences. The
calendar contract is civil-date based: organisation calendar facts and
`Deadline.dueDate` are PostgreSQL `DATE` columns and cross the API boundary as
exact `YYYY-MM-DD` strings. They are dates rather than instants.

| Field | Type | Purpose |
| --- | --- | --- |
| `title`, `description` | `String` / `String?` | Deadline detail. |
| `dueDate` | `DateTime @db.Date` | Exact civil due date. |
| `scheduleVersion` | `Int` (default `1`) | Manual reminder-occurrence identity; increments on reschedule or reopen. |
| `isAutoGenerated` | `Boolean` (default `false`) | Distinguishes derived occurrences from manual deadlines. |
| `generatedKind`, `generatedKey`, `generationVersion` | nullable | Stable rule identity and append-only occurrence version. |
| `generationRuleVersion`, `generationFingerprint` | nullable | Implemented rule version and deterministic SHA-256 provenance fingerprint. |
| `generationSource`, `generationInputs` | `Json?` | Frozen source classifications, warnings, professional-review flag and exact inputs. |
| `profileRuleKey` | `String?` | Stable identity for a manual conditional-profile review deadline. |
| `isComplete`, `completedDate`, `completionDateKnown` | Boolean / nullable instant / Boolean | Truthful completion state, including retained legacy completions whose date was never recorded. |
| `reminderDays` | `Int[]` (default `[30, 14, 7]`) | Days-before reminder offsets. |
| `supersededAt`, `supersededById`, `supersessionReason` | nullable | Generated-occurrence lifecycle and successor provenance. |
| `archivedAt` | `DateTime?` | Soft archive for manual deletion. |

### Confirmed calendar evidence

P0-06 removes the former default CLG assumption. `Organisation.legalForm` is
nullable and company rules require both `legalForm === CLG` and a non-null
`legalFormConfirmedAt`. The company profile separately records incorporation,
member count, the last AGM that actually occurred, and the last unanimous
annual member resolution; a written resolution never overwrites the actual AGM
fact. CRO generation additionally requires the exact ARD copied from CORE and
a non-null `croAnnualReturnDateConfirmedAt`.

`OrganisationService.updateOrganisation()` validates exact date ranges,
future-event facts, and incorporation ordering. It performs the organisation
update and `reconcileGeneratedDeadlines()` in the same serializable transaction,
locking the organisation row and retrying transaction conflicts up to three
times. An unchanged legal form or ARD may be explicitly unconfirmed while its
value is retained; this clears the confirmation timestamp and supersedes the
dependent generated occurrence. A changed non-null legal form or ARD must be
freshly confirmed.

### Implemented rule set and provenance

`deriveIrishGovernanceDeadlines()` in
`apps/api/src/services/deadline-calendar.ts` currently produces four stable rule
kinds. Every result is a planning/review date with
`professionalReviewRequired: true`; citing statutory text does not turn the
software output into legal advice or proof of the live portal date.

| Kind | Required evidence and calculation | Provenance / limitation |
| --- | --- | --- |
| `CHARITY_ANNUAL_REPORT` | Financial year end plus ten calendar months, clamping a missing month-end day. | Revised Charities Act section 52 is stored as a statutory source. Month-end clamping is an explicit planning convention, and regulator-approved extensions are never inferred. |
| `COMPANY_FINANCIAL_STATEMENTS` | Confirmed CLG plus financial year end; nine calendar months, clamped and then advanced for Saturday, Sunday or covered Irish public holidays. | Revised Companies Act sections 341 and 3 plus public-holiday sources. It is distinct from CRO annual-return filing and from meeting notice/circulation lead time. |
| `COMPANY_ANNUAL_MEMBER_ACTION` | Confirmed CLG. Uses an eligible latest sole-member written resolution plus 12 months as an internal review cadence; otherwise the earlier of last actual AGM + 15 months and the end of the following calendar year, or incorporation + 18 months when no AGM is recorded. Statutory branches receive the working-day adjustment. | The 12-month sole-member path is explicitly non-statutory and requires confirmation of every eligibility condition. Other branches cite revised Companies Act sections 175/1202 and working-day sources. A multi-member or unknown-member CLG never advances the AGM calculation from a written resolution. |
| `CRO_ANNUAL_RETURN` | Confirmed exact ARD; the earlier of ARD + 56 days and, when available, financial year end + nine months + 56 days, followed by the working-day adjustment. | CRO guidance and revised Companies Act sections 343/3 are stored. The output is review-only: earlier made-up dates, first returns, altered ARDs, court extensions and other exceptions require CORE/accountant confirmation. |

Each occurrence freezes the source authority, title, URL, check date and
classification (`statutory`, `official-guidance`, or
`derived-planning-convention`), its calculation inputs, warnings, rule version
and fingerprint. Provenance separates authority from output certainty: a source
may be statutory while the derived CharityPilot date remains a provisional
planning/review result. Civil arithmetic is implemented in shared calendar helpers;
the deadline engine rejects invalid non-null civil inputs rather than coercing
JavaScript `Date` values.

### Versioning and mutation semantics

Generated occurrences are not edited in place. Reconciliation compares stable
`generatedKey`, due date, rule version and fingerprint under the organisation
lock:

- unchanged current occurrences are left untouched, including completed ones;
- changed inputs or rules supersede the old row and create a new incomplete
  successor with the next `generationVersion`;
- removed or unconfirmed evidence supersedes the current row without inventing
  a successor;
- legacy generated rows are migrated to immutable, superseded
  `LEGACY_UNVERIFIED` history because their original inputs and sources were not
  recorded.

Generated rows reject edit, reopen, reschedule and delete operations; a single
atomic `false → true` completion is allowed. The next occurrence appears only
when newly recorded verified evidence changes the rule output. Manual deadlines
remain editable, use soft archive rather than hard delete, and may be reopened.
Rescheduling or reopening a manual row increments `scheduleVersion`, separating
future reminder identity from earlier delivery attempts. Completion records a
real instant; the migration uses `completionDateKnown: false` rather than
fabricating a timestamp for completed legacy rows whose completion time is
unknown.

### Listing and access

`GET /deadlines` is paginated (default 50, maximum 100) and returns only
non-superseded, non-archived current lifecycle rows, ordered incomplete first,
then by civil due date and id. `GET /deadlines/history` is separately paginated
and returns completed or superseded generated occurrences, excluding open
current generated rows. `GET /deadlines/reminder-history` is admin-only,
paginated, filterable by deadline/status, and returns the immutable title, due
date and schedule-version snapshots captured by each attempt rather than live
edited deadline fields.

Authenticated subscribed members may read the current list and generated
history. Create, update and delete require `OWNER` or `ADMIN`; cross-tenant ids
fail as `404 DEADLINE_NOT_FOUND`. Database partial unique indexes backstop one
current generated occurrence per rule key and one incomplete current manual
deadline per conditional-profile rule key.
