...
- [ ] `INTEGRATION_ENCRYPTION_KEY` is an independent canonical 32-byte hex/base64url secret in the approved secret store and is not reused as `JWT_SECRET`, `OWNER_JWT_SECRET`, `AUTH_RECOVERY_SECRET`, or `READINESS_API_KEY`. It seals every stored charity integration credential, so it carries a harsher loss consequence than its peers: losing `JWT_SECRET` only logs everyone out, but losing or rotating this key without first re-sealing makes every stored charity credential permanently unreadable, recoverable only by re-authorising every connected integration.
