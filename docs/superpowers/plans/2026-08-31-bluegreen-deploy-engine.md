...
- [ ] **Step 1:** Write `.bluegreen/local.env` (self-contained axes per P1, scratch secrets ≥32 chars, `DATABASE_URL` at the compose db, `BLUEGREEN_ENV_FILE` pointing at itself). `npm run bluegreen:deploy -- --env-file .bluegreen/local.env` (skip-backup NOT set — prove backup too). Evidence: status file phases, `docker compose ps` showing blue serving, public smoke output with buildCommit.
...
- [ ] **Step 4:** `npm run bluegreen:restore-drill` against the step-1 backup → census + hashes verified, scratch container gone after.
