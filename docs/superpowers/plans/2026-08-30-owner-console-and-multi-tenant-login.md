...
- **Never assert an error code with a bare regex.** `AppError(status, code, message)` puts the code on `.code`, not in the message, so `assert.rejects(fn, /SOME_CODE/)` matches the *message* and can never pass. Several test blocks below were written that way and are wrong. Follow the repo convention instead — `const codeOf = (err: unknown) => (err as { code?: string })?.code;` then `assert.rejects(fn, (e: unknown) => codeOf(e) === 'SOME_CODE')`, as used in `apps/api/src/tests/team-reliability.test.ts:29`.
- **In-memory Prisma stubs must honour the `where` clause they are given.** A stub that ignores `where` and returns a fixed row — or that reimplements the filter with its own hardcoded predicate — tests the stub, not the service: the test still passes when the production query drops a filter condition. Where a task's security property depends on a query filter (`revokedAt: null`, `expiresAt: { gt: now }`, `organisationId`, `lifecycleStatus`), the stub must apply the passed filters and the test must drive the negative case with real row data rather than by injecting `null`. This was found as a genuine defect in Tasks 3 and 4 and applies to every stub in this plan.
