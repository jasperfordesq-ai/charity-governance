...
Working-tree base commit when generated: `15b5091`
...
- Compliance standard autosave, organisation profile saving, and governance register saving now use a shared save-status primitive for saving, saved, and failed states instead of route-local status markup.
...
- `npm run lint -w @charitypilot/web`, `npm run build -w @charitypilot/web`, and `npm run test:production-check` passed locally after the shared blog empty-state, compliance save-status, organisation save-status, and register save-status primitive cleanup.
