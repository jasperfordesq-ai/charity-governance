...
test('clearing organisation profile dates supersedes generated deadlines without touching manual records', async () => {
  ...
  assert.deepEqual(updates[0], {
    ...
    data: {
      supersededAt: (updates[0] as { data: { supersededAt: Date } }).data.supersededAt,
      supersessionReason: 'INPUT_REMOVED',
    },
  });
  assert.ok((updates[0] as { data: { supersededAt: Date } }).data.supersededAt instanceof Date);
