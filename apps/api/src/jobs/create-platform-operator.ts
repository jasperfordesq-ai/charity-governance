...
// Constructs the one-time operator set-password link with token in URL fragment,
// never the query string (fragments do not reach servers, proxies, or access logs).
// Token must be the raw, unhashed reset token generated by the bootstrap CLI.
export function buildOperatorSetPasswordLink(origin: string, resetToken: string): string {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    process.stdout.write(`${buildOperatorSetPasswordLink(origin, resetToken)}\n`);
  } finally {
