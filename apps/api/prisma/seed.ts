...
async function upsertDeadline(organisationId: string, data: {
  ...
  const existing = await prisma.deadline.findFirst({
    where: { organisationId, title: data.title },
  });
...
async function seedDemoWorkspace(config: EnabledLocalAdminSeedConfig) {
  ...
  if (!organisation) {
    organisation = await prisma.organisation.create({
      data: {
        ...
        lastAgmDate: dateFromIso('2026-03-12'),
      },
    });
  } else {
  ...
  await upsertDeadline(organisation.id, {
    ...
    description: 'File the charity Annual Report with the Charities Regulator within 10 months of financial year end.',
    dueDate: dateFromIso('2026-10-31'),
